...
`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		dirsToGet := []string {"wal", "digitalrebar", "secrets", "data"}
...
func AddDirToZip(zipWriter *zip.Writer, dirname string) error {
	...
	if err != nil {
		...
		if strings.HasSuffix(dirname, "data/") { return nil }
	...
	for _, file := range files {
		...
		if !file.IsDir() {
			fmt.Println("Adding " + file.Name() + " to zip")
			...
			// Recurse
			fmt.Println("found a dir. Diving into it.")
